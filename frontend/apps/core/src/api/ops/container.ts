...
// 容器
export const getContainers = async (params?: any) => normalizeContainerList(unwrap(await request.get('/api/containers', { params })))
...
// 镜像
export const listImages = async () => unwrap(await request.get('/api/containers/images'))
...
// 总览
export const getDockerOverview = async () => normalizeDockerOverview(unwrap(await request.get('/api/containers/overview')))

// 网络
export const listDockerNetworks = async () => unwrap(await request.get('/api/containers/networks'))
export const removeNetwork = (id: string) => request.delete(`/api/containers/networks/${sanitizeDockerResourceId(id)}`)

// 存储卷
export const listDockerVolumes = async () => unwrap(await request.get('/api/containers/volumes'))
...
// 镜像仓库
export const listRegistries = async () => unwrap(await request.get('/api/containers/registries'))
...
// 编排项目
export const listComposeProjects = async () => unwrap(await request.get('/api/containers/compose'))
...
// 编排模板
export const listComposeTemplates = async () => unwrap(await request.get('/api/containers/templates'))
